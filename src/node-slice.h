...
class NodeSlice {
  public:
    ...
    iterator begin() { return NodeSliceIterator(g, g->first_out[u]); }
    const_iterator begin() const { return NodeSliceIterator(g, g->first_out[u]); }
    iterator end() { return NodeSliceIterator(g, g->first_out[u + 1]); }
    const_iterator end() const { return NodeSliceIterator(g, g->first_out[u + 1]); }
