...
struct __attribute__((aligned(4 * SIMD_LEN))) aligned_array : public std::array<Type, size>
{
};
...
class BellmanFordSIMD
{
  public:
    ...
    void generic_compute_distance(const std::array<NodeId, SIMD_LEN> &start_nodes, auto neighbors, auto for_all_nodes)
    {
        reset();
        ...
        for (uint32_t i = 0; i < number_of_nodes - 1 && changed; i++)
        {
            ...
            auto relax_neighbors = [&](NodeId v) {
                __m256i vec_v = _mm256_load_si256((__m256i_u *)(distance[v].data()));
                auto relax_operation = [&](NodeId u, Distance w) {
                    aligned_array<Distance, SIMD_LEN> e_w = {w, w, w, w, w, w, w, w};

                    // 8x 32-bit integer into simd register
                    __m256i vec_u = _mm256_load_si256((__m256i_u *)(distance[u].data()));
                    __m256i vec_weight = _mm256_load_si256((__m256i_u *)(e_w.data()));
                    ...
                    _mm256_store_si256((__m256i_u *)distance[u].data(), vec_min);
  ...
  private:
    void init_start_nodes(const std::array<NodeId, SIMD_LEN> &start_nodes)
    {
    ...
    std::vector<aligned_array<Distance, SIMD_LEN>> distance;
