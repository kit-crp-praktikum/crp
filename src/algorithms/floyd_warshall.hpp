...
class FloydWarshall {
    public:
        FloydWarshall(std::size_t size) : 
            number_of_nodes(size),
            distance(size * size, INF), 
            parent(size * size, INF) {}


        template<bool update_parents = false>
        void compute_all_distances(auto neighbors) {
            reset();
                ...
                for(NodeId j = 0; j < n; j++) {
                    for(NodeId k = 0; k < n; k++) {
                        ...
                        if(alternative < distance[get_index(i, j)]) {
                            ...
                            if constexpr(update_parents) parent[get_index(i, j)] = parent[get_index(i, k)];
                        }
                    }
                }
            }
        ...
        //set number of nodes if you want to operate on a subgraph of smaller size. NodeIDs have to be mapped to 0, ..., n - 1
    ...
    private:
        ...
        TimestampedVector<Distance> distance; //represent 2d array as rolled out 1d array to use timestamped vector without modification
